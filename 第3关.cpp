...
#define WID 800 //这一关的环境尺寸和别的不一样，因为四周是雪山
#define HEI 800 //同上
...
void Game::FunState3()
{
	if (enemyCount == 4) // 这里测试是否过关，如果声波和雪崩都不存在，并且敌人被全部消灭则过关
	{
		...
		if (snow_exist == false)
		{
			text = L"恭喜过关！BGM:嘉蒂外传通关后；下一关：王的沉默";
			goto win;
		}
	}
	...
	if (my != NULL && gameOver != 1)
	{
		...
		BitBlt(hDc, 50, 20, WID, HEI, mDc, 0, 0, SRCCOPY);
...
bool Game::InitLevel3()
{
	...
	speedTemp = 6;
	backGround = (HBITMAP)::LoadImage(NULL, L"image\\白色2.bmp", IMAGE_BITMAP, WIDTH, HEIGHT, LR_LOADFROMFILE); //读取这关游戏背景图
	//以下四行读取这关敌人图像
	enemyDirection0[0] = (HBITMAP)LoadImage(NULL, L"image\\3up.bmp", IMAGE_BITMAP, LENGTH, LENGTH, LR_LOADFROMFILE); 
	enemyDirection0[1] = (HBITMAP)LoadImage(NULL, L"image\\3right.bmp", IMAGE_BITMAP, LENGTH, LENGTH, LR_LOADFROMFILE);
	enemyDirection0[2] = (HBITMAP)LoadImage(NULL, L"image\\3down.bmp", IMAGE_BITMAP, LENGTH, LENGTH, LR_LOADFROMFILE);
	enemyDirection0[3] = (HBITMAP)LoadImage(NULL, L"image\\3left.bmp", IMAGE_BITMAP, LENGTH, LENGTH, LR_LOADFROMFILE);
	...
	SelectObject(bufDc, backGround); //以下三句贴图代码，完全是为了刷出来四周雪山背景
	BitBlt(mDc, 0, 0, WIDTH, HEIGHT, bufDc, 0, 0, SRCCOPY);
	BitBlt(hDc, 0, 0, WIDTH, HEIGHT, mDc, 0, 0, SRCCOPY);

	backGround = (HBITMAP)::LoadImage(NULL, L"image\\backGround1.bmp", IMAGE_BITMAP, WIDTH, HEIGHT, LR_LOADFROMFILE);//完事重新读取黑色背景
	my = new Tank(100, 100, 0, 4, 100, false);
	for (int i = 0; i < ENEMYALL; i++) //这里是初始化敌军位置和状态
	{
		enemy[i] = new Tank(i* LENGTH*3+LENGTH, (i) * LENGTH*2+LENGTH, 0, 1, 100, false);
	}
	enemy[ENEMYALL] = new Tank(HEI / 2 - LENGTH / 2, WID / 2 - LENGTH / 2, 0, 0, 0, false);
	myBullet = NULL;
	gameOver = 0;
	enemyCount = 0;
	PlaySound(L"music\\3.wav", NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
	return false;
}